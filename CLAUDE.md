...
The worker is the gap-killer: when a jam starts, a `std::thread::spawn` takes ownership of the `JamSession` and loops `render_next() → mpsc::sync_channel(1).send()`. Capacity 1 means the worker is always either holding the next section ready or mid-render of it. When the main thread consumes a section, the worker immediately renders the one after — so by the time playback of the current section finishes, the next section's PCM has been sitting in the channel for ~29 seconds. The transition is gapless in practice (single cpal stream rebuild, no render latency in the critical path). Drop the receiver to stop: the worker's next `send` returns Err and the thread exits.
