...
| `memdeck_validate_abc` | Validate an ABC text blob passed as a string (no disk write). Returns `{ok, voice_count, pattern_count, arrangement_length, bpm}` on success or `{ok: false, error}` on failure. Use in a tight draft-and-validate loop. |
